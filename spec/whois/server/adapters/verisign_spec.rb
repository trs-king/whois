  ...
  describe "#lookup" do
    context "without referral" do
      it "returns the WHOIS record" do
        ...
        expected = response
        expect(server.query_handler).to receive(:execute).with("=example.test", "whois.test", 43).and_return(response)
    ...
    context "with referral" do
      it "follows all referrals" do
        ...
        expected = referral + "\n" + response
        expect(server.query_handler).to receive(:execute).with("=example.test", "whois.test", 43).and_return(referral)
        expect(server.query_handler).to receive(:execute).with("example.test", "whois.markmonitor.com", 43).and_return(response)
      ...
      it "extracts the closest referral if multiple referrals" do
        ...
        expect(server.query_handler).to receive(:execute).with("=example.test", "whois.test", 43).and_return(referral)
        expect(server.query_handler).to receive(:execute).with("example.test", "whois.markmonitor.com", 43).and_return("")
