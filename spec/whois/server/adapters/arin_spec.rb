  ...
  describe "#lookup" do
    context "without referral" do
      it "returns the WHOIS record" do
        ...
        expected = response
        expect(server.query_handler).to receive(:execute).with("n + 0.0.0.0", "whois.arin.net", 43).and_return(response)
    ...
    context "with referral" do
      it "follows whois:// referrals" do
        ...
        response = "Whois Response"
        expected = referral + "\n" + response
        expect(server.query_handler).to receive(:execute).with("n + 0.0.0.0", "whois.arin.net", 43).and_return(referral)
        expect(server.query_handler).to receive(:execute).with("0.0.0.0", "whois.ripe.net", 43).and_return(response)
      ...
      it "follows rwhois:// referrals" do
        ...
        expected = referral + "\n" + response
        expect(server.query_handler).to receive(:execute).with("n + 0.0.0.0", "whois.arin.net", 43).and_return(referral)
        expect(server.query_handler).to receive(:execute).with("0.0.0.0", "rwhois.servernap.net", 4321).and_return(response)
      ...
      it "ignores referral if options[:referral] is false" do
        ...
        expect(server.query_handler).to receive(:execute).with("n + 0.0.0.0", "whois.arin.net", 43).and_return(referral)
        expect(server.query_handler).to receive(:execute).with("0.0.0.0", "whois.ripe.net", 43).never
      ...
      it "ignores referral (gracefully) if missing" do
        ...
        expect(server.query_handler).to receive(:execute).with("n + 0.0.0.0", "whois.arin.net", 43).and_return(referral)
        expect(server.query_handler).to receive(:execute).never
      ...
      it "folows referrals without ports" do
        ...
        response = "Whois Response"
        expect(server.query_handler).to receive(:execute).with("n + 0.0.0.0", "whois.arin.net", 43).and_return(referral)
        expect(server.query_handler).to receive(:execute).with("0.0.0.0", "whois.apnic.net", 43).and_return(response)
