  ...
  describe "#lookup" do
    context "without referral" do
      it "returns the WHOIS record" do
        ...
        expected = response
        expect(server.query_handler).to receive(:execute).with("example.test", "whois.afilias-grs.info", 43).and_return(response)
    ...
    context "with referral" do
      it "follows all referrals" do
        ...
        expected = referral + "\n" + response
        expect(server.query_handler).to receive(:execute).with("example.test", "whois.afilias-grs.info", 43).and_return(referral)
        expect(server.query_handler).to receive(:execute).with("example.test", "whois.belizenic.bz", 43).and_return(response)
      ...
      it "ignores referral if options[:referral] is false" do
        ...
        expect(server.query_handler).to receive(:execute).with("example.test", "whois.afilias-grs.info", 43).and_return(referral)
        expect(server.query_handler).to receive(:execute).with("example.test", "whois.belizenic.bz", 43).never
