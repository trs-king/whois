...
describe Whois::Server::Adapters::Standard do
  ...
  describe "#lookup" do
    it "returns the WHOIS record" do
      ...
      expect(server.query_handler).to receive(:execute).with("domain.test", "whois.test", 43).and_return(response)
    ...
    context "with port option" do
      it "sends the request to given port" do
        ...
        expect(server.query_handler).to receive(:execute).with("domain.test", "whois.test", 20).and_return(response)
    ...
    context "with bind option" do
      it "binds the request to given host and port" do
        ...
        expect(server.query_handler).to receive(:execute).with("domain.test", "whois.test", 20, "192.168.1.100", 3000).and_return(response)
